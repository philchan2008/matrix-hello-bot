// Configuration
var config = require('../matrix-bot-config.js').redmine;
...
// var findRedmineIssues = function(matrix_user, search) {
//   var deferred = q.defer();
//
//   getUserKey(matrix_user).then(
//     function(userKey) {
//       console.log('Redmine API request: /redmine/issues.json?key=' + encodeURIComponent(userKey.api_key));
//       request({ url: config.redmine_url + '/redmine/issues.json?key=' + encodeURIComponent(userKey.api_key), json: true, method: 'GET' },
//         function(err, res, body) {
//           if(err || !Array.isArray(body)) {
//             console.log('Redmine API error:');
//             console.log(err);
//             console.log('Redmine API response body:');
//             console.log(body);
//             deferred.reject('An error occured accessing the Redmine API.');
//           } else {
//             console.log('Redmine API response:');
//             console.log(body);
//
...
exports.runQuery = function(client, query, querySender, queryRoom) {
  ...
  console.log('Redmine: Received query "' + query + '"...');
    ...
    if (req[0] == 'login') {
            ...
            if (api_key) {
              ...
              db.run("INSERT INTO auth_requests (id, matrix_user, room_id, requested_at) VALUES (?, ?, ?, ?)", auth_request, querySender, queryRoom.roomId, new Date(), function (err) {
                if (err) {
                  ...
                  deferred.resolve('Please provide your Redmine API token here: ' + config.myServer + 'time_entries.json?key=' + encodeURIComponent(auth_request));
                }
              });
            }
      ...
      db.run('DELETE FROM api_keys WHERE matrix_user=?', querySender, function (err) {
        if (err) {
          ...
          client.sendDM(querySender, 'An error occured while trying to remove your Redmine API key from our database. Please try again later.');
        } else {
